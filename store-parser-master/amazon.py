...
class AmazonParser(Parser):
    ...
    def extract_results(self, markup):
        ...
        traverse = HtmlTraversal(markup)
        elements = traverse.get_elements(
            "div", {"class": "a-fixed-left-grid-inner"})

        for x in elements:
            result = Result()
            result.title = traverse.in_element(x).get_value_of(
                "h2", {"class": "s-access-title"})
            result.url = traverse.in_element(x).get_attr_of(
                "a", {"class": "a-link-normal a-text-normal"}, "href")
            result.price = traverse.in_element(x).get_value_of(
